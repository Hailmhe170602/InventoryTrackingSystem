...
<c:set var="pageTitle" value="Quản lý Dòng sản phẩm" scope="request"/>
...
<div class="subpage-container">
  <div class="subpage-header" style="display: flex; justify-content: space-between; align-items: flex-end; margin-bottom: 24px;">
    <div class="subpage-header__title">
      <h2 style="font-size: 24px; font-weight: 700; color: var(--text-primary); margin: 0;">Quản lý Dòng sản phẩm</h2>
      <p style="font-size: 14px; color: var(--text-secondary); margin: 0;">Phân loại sản phẩm theo từng hãng.</p>
    </div>
    <c:if test="${currentUser.hasPermission('PRODUCT_LINE_WRITE')}">
    <div>
      <a href="${pageContext.request.contextPath}/admin/product-lines?action=create" class="premium-btn-primary" style="display: inline-flex; align-items: center; justify-content: center; gap: 8px; text-decoration: none; height: 44px; line-height: 44px; box-sizing: border-box;">
  ...
  <div class="premium-card" style="padding: 32px;">
    <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 24px; padding-bottom: 16px; border-bottom: 1px solid var(--card-border);">
      ...
      <div>
        <h3 style="margin: 0; font-size: 18px; font-weight: 700; color: var(--text-primary);">Danh sách Dòng sản phẩm</h3>
      </div>
    </div>
    
    <!-- Search Bar -->
    <div style="display: flex; gap: 16px; margin-bottom: 24px; flex-wrap: wrap; align-items: center; justify-content: space-between;">
      <form onsubmit="submitSearch(event)" style="display: flex; gap: 12px; flex: 1; min-width: 300px; max-width: 500px; position: relative; margin: 0;">
        <span style="position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: var(--text-secondary); font-size: 16px;">⌕</span>
        <input type="text" id="search-input" value="<c:out value="${search}"/>" placeholder="Tìm kiếm theo Mã hoặc Tên dòng..." 
        ...
        <tbody>
          <c:forEach var="p" items="${productLines}">
            <tr class="user-row">
              ...
              <td>
                <span class="premium-tag premium-tag--admin">${p.brand.name}</span>
              </td>
              <td>${p.description}</td>
              <td style="text-align: center; vertical-align: middle;">
                <div class="action-dropdown-container" style="position: relative; display: inline-block; text-align: left;">
                  ...
                  <div class="action-dropdown-menu" style="display: none; position: absolute; right: 0; top: 40px; background: #ffffff; border: 1.5px solid var(--card-border); border-radius: 10px; box-shadow: 0 10px 25px rgba(0, 0, 0, 0.08); z-index: 100; min-width: 160px; overflow: hidden; animation: slideDown 0.15s ease-out;">
                    <a href="${pageContext.request.contextPath}/admin/product-lines?action=view&id=${p.id}" class="action-dropdown-item" style="display: flex; align-items: center; gap: 8px; padding: 12px 16px; font-size: 13px; font-weight: 600; color: var(--text-primary); text-decoration: none; transition: background 0.15s;">
                    ...
                    <c:if test="${currentUser.hasPermission('PRODUCT_LINE_WRITE')}">
                      <a href="${pageContext.request.contextPath}/admin/product-lines?action=edit&id=${p.id}" class="action-dropdown-item" style="display: flex; align-items: center; gap: 8px; padding: 12px 16px; font-size: 13px; font-weight: 600; color: var(--text-primary); text-decoration: none; transition: background 0.15s;">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                      ...
                      <form method="post" action="${pageContext.request.contextPath}/admin/product-lines" style="margin: 0;" onsubmit="return confirm('Bạn có chắc chắn muốn xóa dòng sản phẩm này?');">
